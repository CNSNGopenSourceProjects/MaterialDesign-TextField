package br.com.conseng.textfield
...
class MainActivity : AppCompatActivity() {
    ...
    fun validateForm(view: View) {
        if (editTextEmail.text.toString().isEmpty()) {
            ...
            textLayoutEmail.error = getString(R.string.preencher_com_email)
        } else if (!android.util.Patterns.EMAIL_ADDRESS.matcher(editTextEmail.text).matches()) {
            textLayoutEmail.isErrorEnabled = true
            textLayoutEmail.error = getString(R.string.email_invalido)
        } else {
            textLayoutEmail.isErrorEnabled = false
        }
        if (editTextPassword.text.toString().isEmpty()) {
            textLayoutPassword.isErrorEnabled = true
            textLayoutPassword.error = getString(R.string.preencher_com_senha)
        } else {
